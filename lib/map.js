...
Map = function() {
    ...
    this.calculateDestination = function(pathToCheck, moveKind) {
        ...
        console.log("calculateDestination is called properly")
        if(moveKind == 'forward') { //forward movement
            console.log("it realizes that it wants to move forward")
            if (pathClear != true) { //Only occurs if path is not clear.
                console.log("it thinks that the path is not clear?")
            ...
            console.log("its getting here, so it should return the last square!?")
            return pathToCheck[pathToCheck.length -1];
        }
        else{ 
            //moveKind is backward or sideways 
            if (pathClear) {
    ...
    this.moveShip = function(ship, endPosition){
        ...
        if (normalizedMovementVector[0] == ship.orientation[0] && normalizedMovementVector[1] == ship.orientation[1]){
            ...
            this.moveStraight(ship, Math.abs(movementVector[0] + movementVector[1]));
        }
        ...
        //movement is sideways
            ...
            if (ship.orientation[0] == 0){
                this.moveSideways(ship,normalizedMovementVector[0]);
            }

            else if (ship.orientation[1] == 0){
                this.moveSideways(ship,normalizedMovementVector[1]);
            }
        }
    ...
    this.moveStraight = function(ship, distance){
        ship.sternPosition[0] += distance*ship.orientation[0];
        ship.sternPosition[1] += distance*ship.orientation[1];
        this.redrawShip(ship);

    }

    this.moveSideways = function(ship, distance){
        ship.sternPosition[0] += distance*ship.orientation[1]*ship.orientation[1];//faster than absolute value
        ship.sternPosition[1] += distance*ship.orientation[0]*ship.orientation[0];
        this.redrawShip(ship);
    }
    ...
    this.redrawShip = function(ship){
        ...
        console.log('ship stern pos is now');
        console.log(ship.sternPosition);
    ...
    this.turnShip = function(ship, radians){
        var x = ship.orientation[1]*Math.cos(radians) - ship.orientation[0]*Math.sin(radians);
        var y = ship.orientation[1]*Math.sin(radians) + ship.orientation[0]*Math.cos(radians);
        console.log(x + 'and' + y);
        ship.orientation[1] = Math.round(x);
        ship.orientation[0] = Math.round(y);
