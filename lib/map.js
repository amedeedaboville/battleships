...
//        this.shipDictionary["kamikaze2"] = new Kamikaze([oppos[10],28], "kamikaze2", "opponent");
    ...
    this.moveShip = function(ship, endPosition){
        ...
        if (ship.name == "Kamikaze") {
           ...
           if (dest.name == "sea") {
               ship.shipSquare[0].position = endPosition; // brute force the move because WE SUCK AT PLANNING LOLOLOL
    ...
    */
     ...
     */
    this.kSuicide = function (explosionPosition) {
         var damage = 2; //kamikazes do heavy damage
         for (var i = (x-1); i <= (x+1); i++) {
             for (var j = (y-1); j <= (y+1); j++){
                var curPosition = [i,j];  
                var curObj = getSquareAtPosition(curPosition);
                var objName = curObj.name;
                if (objName == "ship") {
                    var ship = this.shipDictionary[objName.shipName];
