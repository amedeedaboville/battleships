...
Map = function() {
    ...
    this.turnShip = function(ship, turnDirection){
        ...
        for(var i = 1; i < ship.shipLength; i++) {
            for(var j = 1; j < ship.shipLength; j++) { 
                turnArea.push([ship.shipLength - i,j])
            }
        }
        console.log(basicTurnArea);
        if(turnDirection === 'clockwise') {
            var rotationMultiple = 1;
        }
        else {
            var rotationMultiple = -1;
        }
        ...
        if(isAreaClear(turnPositions)) {
            var turnVector = [y[0] * turnMultiple, y[1] * turnMultiple];
            for(var i = 0; i < ship.shipLength; i++) {
                var oldPos = ship.shipSquares[i].position;
                ship.shipSquares[i].position =  [oldPos[0] + i * turnVector[0], oldPos[1] + i * turnVector[1]];
                square.__proto__ = new shipSquare(); //recalculate css props
                square.calculateAttributes();
            }
    ...
    this.fireCannon = function(ship, cannonTarget){
        ...
        this.shipDictionary[targetSquare.shipName].speed--;
