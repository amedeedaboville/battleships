...
Map = function() {
    ...
    this.getGrid = function(player) {
        ...
        for(var r = 0; r < 30; r++) {
            ...
            for(var c = 0; c < 30; c++) {
                var key = '[' + r + ',' + c + ']';
                if(vis[key] == true) {
                    newGrid[r][c] = this.grid.squares[r][c];
    ...
    this.getVisibleSquares = function(player) {
        ...
        for (ship in this.shipDictionary){
            ...
            for (var i = 0; i < 24; i++) {
                var spot = this.grid.coralSpots[i];
    ...
    this.fireCannon = function(ship, targetSquare){
        ...
        //If so, create new collision 
        console.log("We are trying to shoot square at position: "+targetSquare.position);
        //Construct AoE

        //TODO: ALL of this should be moved into some "applyCollisions" method in the actions.js file
        targetSquare.health -= 1;
        if (targetSquare.health === 0)
            targetSquare.isDestroyed = 'true';
        console.log("shot happened.");
        serverStream.emit('message');
...
//        $('document').ready(function(){alert("Ship was hit!")});
        if ( targetSquare== 'sea' && targetSquare == 'coral' ){
        this.shipDictionary[targetSquare.shipName].speed =  this.shipDictionary[targetSquare.shipName].speed -2; //*= (4/ship.shipLength);
    ...
    this.fireTorpedo = function(ship){
        ...
        //calculateDestination(,"forward")
        //Check if targetSquare is the broadside of a ship (basically jsut check if === bowSquare || === stern
        //Generate AoE and return
    }
    ...
    this.dropMine = function(ship, mineLocation){
        if (ship.shipType != "Mine Layer")
            return;
        //Get "mine zone" around mineLayer ship
    }
