...
Map = function() {
    ...
    this.makeShips = function(rand, challenger, opponent) {
        ...
        if (rand){
            ...
            if (opponent){
                ...
                this.shipDictionary["kamikaze2"] = new Kamikaze([oppos[10],28], "kamikaze2", "opponent", [0,-1]);
    ...
    */
    ...
    this.damageShip = function(shipId, position, damage){
            var damageDone = damage || 1;
            var ship = this.shipDictionary[shipId];
            ship.__proto__ = new Ship();
            ship.takeDamage(position,damageDone);
            ship.calculateAttributes();
        }

    this.damageShipSpread = function(shipId, position, damage){
            var damageDone = damage || 1;
            var ship = this.shipDictionary[shipId];
            ship.__proto__ = new Ship();
            ship.takeDamageSpread(position,damageDone);
            ship.calculateAttributes();
        }
    ...
    this.explodeMine = function(position){
        ...
        if (square && square.mine){
            ...
            this.damageShip(square.shipId, position, 2);
            this.damageShipSpread(square.shipId, position, 2);
