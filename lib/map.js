...
Map = function() {
    ...
    this.getVisibleSquares = function(player){
     ...
     for (ship in shipDictionary){
        if (shipDictionary[ship].owner == player){


            //Funny math in order to get the zone to append onto the ship properly. Note it only works with radars with width 3!
            var zoneStart = shipDictionary[ship].stern;
            zoneStart = [zoneStart[0] + (-2)*shipDictionary[ship].orientation[1], zoneStart[1] + (-2)*shipDictionary[ship].orientation[0]];

            //Create a zone for the ship in question!
            var z = new Zone(shipDictionary[ship].orientation, shipDictionary[ship].radarRange, zoneStart);
            var shipSquares = z.enumerateSquares();
            ...
            for(square in shipSquares){
                ...
                //if the ship is owned by the player, get the ship's radar zone squares. 
            //Put it in a hash or something so that you can store all of them
        }
    }
    ...
    this.moveShip = function(ship, endPosition){
        ...
        //Calculate vector
        var movementVector = [endPosition[0] - startPosition[0],endPosition[1] - startPosition[1]];

        //Normalize
        movementVector[0] = Math.sign(movementVector[0]);
        movementVector[1] = Math.sign(movementVector[1]);

        //Get dot product to determine direction based on orientation and movement vector
        var dot = movementVector[0]*this.orientation[0] + movementVector[1] * this.orientation[1];

        var moveKind;

        switch(dot) {
            case 1:
                //Ship is moving forward -- generate a whole path based on the distance.
                moveKind = 'forward';
       ...
       shipToMove.move(this.grid.calculateDestination(path), moveKind); 
