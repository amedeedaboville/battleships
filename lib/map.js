...
Map = function() {
    ...
    this.moveShip = function(ship, endPosition){
        ...
        console.log("moveKind is ");
        console.log(moveKind);
        shipToMove.bowPosition = this.grid.calculateDestination(path, moveKind).position;
        ...
        console.log("new bow position");
        console.log(shipToMove.bowPosition);
        console.log("new stern position");
        console.log(shipToMove.sternPosition);

        shipToMove.shipSquares.forEach(function(square) {
            var oldPos = square.position;
            square.position =  [oldPos[0] + displacementVector[0], oldPos[1] + displacementVector[1]];
        ...
        this.drawGrid();
        console.log(shipToMove.shipSquares);
        //TODO:need to make a 'collision' here if the ship moves over a mine.
    };

    //Usage: turnShip(Ship ship, String turnDirection)
    this.turnShip = function(ship, endPosition, turnDirection){
        var x = ship.orientation;
        var y = [ship.orientation[1], ship.orientation[0]];
        var bowPosition = ship.bowPosition;
        var turnArea = ;
        for (var i = 1; i <= ship.length; i++){
        for (var i = 1; i <= j; i++){
            //The x coordinate will equal i, the y coordinate will equal length -i
            // so as to produce a "diagonal" turn zone,
            // for example [[1,1],[1,2],[1,3],[2,1],[2,2],[3,1]];
            turnArea.push([i,ship.length - i + 1]);
            turnArea.push([ship.length - i + 1, i]);
        }
        //Calculate turnScalar 
        ...
        var turnScalar = 1;
        //Use vector magic to calculate zone~
        var turnPositions = turnArea.map(function(pos) {
                return [
                ship.sternPosition[0] + pos[0] * x[0] + turnScalar * pos[1] * y[0],
                ship.sternPosition[1] + pos[0] * x[1] + turnScalar * pos[1] * y[1]];
                });
    ...
    this.fireCannon = function(ship, cannonTarget){
        ...
        if (typeof targetSquare === 'shipSquare'){
