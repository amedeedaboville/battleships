Grid = function () {
    ...
    this.getDistanceBetween = function(position1, position2){
        ...
        if (x1 === x2){
            ...
            console.log("Error: invalid path (start and end positions not in the same row or column.)");
    ...
    this.calculateDestination = function(pathToCheck, moveKind){
        ...
        switch(moveKind) {
            case 'forward':
                if (resultType.prototype === 'Square') { //Only occurs if path is not clear.
                    var obstructingSquare = result;
                    //Return previous square
                    var squareBefore = pathToCheck.find(function (arg, index, array) {
                ...
                break;
            case 'backward':
                if (resultType === 'boolean') { //The path is clear,so return the single square.
                ...
                break;
            case 'sideways':
                if (resultType === 'boolean') { //The path is clear,so return the first square.
                    return (pathToCheck[0]);
                }
